                ...
                if (data.showPathOptions)
                {
                    ...
                    if (handleIndexToDisplayAsTransform != -1)
                    {
                        ...
                        using (new EditorGUI.IndentLevelScope())
                        {
                            ...
                            if (GUILayout.Button("Set All Y After to This one"))
                            {
                                var oY = creator.bezierPath[handleIndexToDisplayAsTransform].y;
                                for (int i = handleIndexToDisplayAsTransform + 1; i < creator.bezierPath.NumPoints; i++)
                                {
                                    var p = creator.bezierPath[i];
                                    p.y = oY;
                                    creator.bezierPath.MovePoint(i, p);
                                }
                            }
                        }
                    }
